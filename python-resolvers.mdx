...
only when they are needed, or when they are explicity asked to run. In general, this
means that they will run in response to "queries". A query is just a request for data.
Chalk takes a query and determines which resolvers it needs to run and parallalizes your
code, providing you with a response, quickly. In the next section, we'll dive into queries,
completing.

We will go into more detail about these three different resolver variants and their
particualar use cases, in the next section.

## Python

Python resolvers are python functions perform transformations on your features,
or groups of your features.
