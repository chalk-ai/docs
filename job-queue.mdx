...
The **job queue** in Chalk, together with **resource groups**, functions similarly to **Snowflake warehouses** - they provide dedicated, configurable compute resources for processing workloads.
...
### Fixed Resources
Each job queue server has a single, pre-configured resource allocation (CPU and memory). This contrasts with synchronous queries, which can request arbitrary resources on-demand.
