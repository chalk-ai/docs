...
The [`chalk task run`](/cli/task_run) command executes Python scripts in your deployment:
...
This automatically:
1. Runs [`chalk apply --branch feature/new-algorithm`](/cli/apply#branch)
...
`--skip-apply` - Skip the automatic [`chalk apply`](/cli/apply) when running with `--branch`. Use this if the branch is already deployed.
