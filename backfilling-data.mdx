...
Chalk uses the [`chalk migrate`](/cli/migrate) command to perform this operation. Suppose that you want to add a new column,
...
The first variant will ingest all available historical data. The second will ingest data newer (inclusive)
than the lower bound that you specify with an ISO8601 timestamp.
...
Chalk can automatically compute this feature using historically observed values of `User.name`. To do this, you
use the [`chalk`](/cli/) CLI tool to plan and execute a "migration":
...
and then invoke your resolver. For each sampled tuple `(id, name, observed_at)`, Chalk will write
