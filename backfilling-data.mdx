...
they should have different `ts` values.
...
Chalk uses [offline queries](/docs/training-client) to perform this operation. Suppose you want to add a new column,
...
either use an offline query with [`recompute_features`](/api-docs#ChalkClient.offline_query.recompute_features) set to
True and ingest the computed feature values as we did above, or you can use [`chalk trigger`](/cli/trigger) to trigger a
resolver run.
