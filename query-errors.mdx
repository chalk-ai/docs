...
<AttributeTable>
  ...
  </Attribute> <Attribute field={'feature'} kind={'string?'}> The fully qualified name of the
  failing feature, for example: <code>user.identity.has_voip_phone</code>. </Attribute> <Attribute
  field={'resolver'} kind={'string?'}> The fully qualified name of the failing resolver, for
  example: <code>my.project.get_fraud_score</code>. </Attribute> </AttributeTable>
...
<Attribute field={'VALIDATION_FAILED'} kind={<a href={'#field-error'}>FIELD</a>}> A feature value
did not match the expected schema (for example: `incompatible type "int"; expected "str"`)
</Attribute>
