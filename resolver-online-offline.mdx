...
| Queries the online store and calls `@online` resolvers for quick retrieval                                                    | Queries the offline store which stores all data from offline queries, unless `recompute_features=True` in which case `@offline` and `@online` resolvers are used to resolve the outputs
