...
Two of the core components in Chalk—resolvers, queries—have online and offline variants. Both
offline queries pull data from the offline store. Offline resolvers run in response to offline
queries and can be made to pull data into the offline store.
...
|------------------------------------------------------------------------|-------------------------------------------------------------------------------------|
| an online query writes all features not read from the online store     | online query which computes features with `max_staleness != 0`                      |
| A triggered resolver runs with `store_offline=True` (default behavior) | A triggered resolver runs with `store_online=True` (default behavior)               |
...
| An online or offline scheduled resolver: `@online(cron="*/5 * * * *")`            | An online scheduled resolver: `@online(cron="*/5 * * * *")`                         |
