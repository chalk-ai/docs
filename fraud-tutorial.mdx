...
don't take any features (or only a primary key) as inputs and are used to fetch data from external sources.
...
which is periodically updated in response to changes in PostgreSQL database.
...
We'll start by modeling out the features we want for the users of our system.
...
Next up, we'll define a feature class related to our users. We'll call this class
`Account` and it will represent a bank account that a user owns.
...
that can only be assigned to a single field in a feature class. This field specifies the logical time of a feature instance—it is
used for point in time correctness and incremental updating. We'll cover both later in this tutorial.
...
dashboard, and 2). Define your Data Sources in Your Chalk Code.
...
Note, the connection information you add to the dashboard will be stored securely using
...
the database tooling you're familiar with to test, lint, and debug your
code.
...
compared against the names of the attributes on the specified feature class.
...
-- The features given to us by an account.
-- resolves: Account
-- source: postgres
select
    id,
...
-- The features given to by a transaction.
...
from our [`user.chalk.sql`](https://github.com/chalk-ai/fraud-example/blob/main/src/user.chalk.sql)
resolver. Then, using this account id, Chalk will retrieve `Account.title` from the `account.chalk.sql` resolver.

Lets deploy our code. Note, this time we are using a `branch`. In testing, it can be helpful to deploy your
feature pipeline to a [branch](/cli/apply), which allows you to test your changes without
affecting the production feature pipeline. Branches are not just isolated, they are also incredibly
lightweight—they should deploy pretty much instantly.
...
```sh
chalk apply --branch new-fraud-features
```

```sh
chalk query --in user.id=1 --out user.average_time_between_transactions_30d
```
...
our Chalk online store. At a high level, this means that periodically,
we'll:
1. look for all users with new transactions in the snowflake data source,
...
Of note, we don't want to use our PostgreSQL database for this, we want to use our Snowflake
...
However, data warehouses like [Snowflake](/docs/snowflake) and [BigQuery](/docs/bigquery) are optimized for analytics
...
computed feature sinto the cache
