...
def get_average_time_between_transactions(
  ...
  date_col = df.collect().get_column(str(Transaction.created_at)).dt.timestamp("ms").sort()
...
<TipInfo>
  ...
  fast. When building new features, do some testing. You may not even need to reverse-ETL features into the online
