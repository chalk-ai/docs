...
<TipInfo>
  ...
  In the example above <code>id</code>, <code>birthday</code>, <code>name</code>, <code>email</code>, <code>username</code>, <code>age</code>, and <code>is_adult</code> are all features.
  Features are grouped together (namespaced) in what are called feature
  classes.
  For instance, the <code>User</code> class is a <strong>feature class</strong> (also sometimes referred to as a <strong>feature set</strong>).
  Specific instances of a feature class are <strong>feature class instances</strong>.
</TipInfo>
...
<TipInfo>
  ...
  skip ahead to get a better sense of <a href="#deploying">deploying code to Chalk</a> and <a href="#querying">running queries</a>.
</TipInfo>
...
Chalk handles tracking your features for [temporal consistency](/docs/temporal-consistency), running your resolvers in parallel, and horizontally
...
A [Chalk DataFrame](/docs/dataframe) models tabular data in much the same way that `Pandas` does. However, there are some key
...
To define your data sources, you'll need to create a [`datasources.py`](https://github.com/chalk-ai/fraud-example/blob/main/src/datasources.py) file that contains a [`SnowflakeSource`](/docs/snowflake)
and a [`PostgreSQLSource`](/docs/postgresql):
...
If you want to validate your code without deploying, you can run the [`chalk lint`](/cli/lint) command.
...
To deploy you'll want to run [`chalk apply`](/cli/apply).
...
We can have Chalk [reverse-ETL](/docs/reverse-etl) our offline data into our online store by setting the [`max_staleness`](/api-docs#features.max_staleness)
...
When defining aggregations, it is often useful to compute a [windowed aggregation](/docs/aggregations) to define the
...
[materialized windowed aggregation](/docs/materialized_aggregations). In a materialized windowed aggregation, you can
...
deployments do not persist data to the online store, we would need to run a full deploy with [`chalk apply`](/cli/apply) to use
this new feature. After running a [`chalk apply`](/cli/apply), we can run [`chalk aggregate backfill --feature account.num_transactions`](/cli/aggregate_backfill)
