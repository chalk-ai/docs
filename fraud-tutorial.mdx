...
The building blocks of Chalk are [features](/docs/features) and resolvers.
...
Some might be calculated in real time based on the current timestamp (like the age feature) or a different upstream feature (like the `is_adult` feature, which will depend on the age feature).

You could deploy the code above to Chalk and it would be considered valid.
However, if you tried to query for a field (for example, if you asked chalk to return the name of a user with id 1) then you'd get an error.
...
In particular, the error message tells use that a "Resolver" was not found.
A [resolver](/docs/resolver-overview) is a function that takes in features and outputs features.
...
For example, to read from the `users` table in your PostgreSQL database, you might write a `get_users.chalk.sql` file, with the following contents:
...
By writing this file, you've defined a SQL resolver, named `get_users`, which knows how to resolve `birthday`, `name`, or `email` for any given id in the `users` table.
...
Lets say you've defined the SQL resolver above and deployed it to Chalk.
...
Getting this to work fully, will involve setting up a database connector to Chalk and deploying code, both of which we will cover later.
...
1. Python functions with feature annotations,
...
<TipInfo>
    The advantage of inline expressions is that they are statically compiled into the execution engine, which often provides performance advtantages.
...
Traditional orchestration platforms like [Airflow](https://airflow.apache.org/) or [Dagster](https://dagster.io/)
explicitly compose functions which produce data into a DAG of tasks.
With Chalk, the DAG of [resolvers](/docs/resolver-overview) is defined implicitly by the [features](/docs/features) they produce.
This architecture makes it easy to build out feature pipelines that are reusable and composable.
Chalk handles tracking your features for [temporal consistency](/docs/temporal-consistency), running your resolvers in parallel, and horizontally scaling your feature pipelines.
...
3. [Python Resolvers](/docs/fraud-4) - Defining resolvers in Python that compute derived features and call external APIs.
...
Before you get started, make sure you have the [Chalk CLI installed](/docs/installation).

If you want to skip ahead, you can find the full source code for this tutorial on [GitHub](https://github.com/chalk-ai/fraud-example).
...
First, we'll create a new file called [`feature_sets.py`](https://github.com/chalk-ai/fraud-example/blob/main/src/feature_sets.py) where we'll define a `User` class decorated with [`@features`](/api-docs#features).
...
For example, you may wish to send [Pagerduty alerts](/docs/pagerduty) to [different teams](/docs/alertconfig) based on the owner of the related feature.

All of the comments and tags from the code also show up in the [Chalk dashboard](https://chalk.ai/projects), and are indexed for search.
...
<TipInfo>
  ...
  This is because the Account class is defined later in the file Chalk will recognize this as a valid feature reference and process it correctly.
</TipInfo>
...
A [Chalk DataFrame](/docs/dataframe) models tabular data in much the same way that `Pandas` does. However, there are some key differences that allow the Chalk `DataFrame` to increase type safety and performance.
...
You might also notice that we used an [`Enum`](/docs/feature-types#scalars) feature.
Chalk supports many feature types, including [`Enums`](/docs/feature-types#scalars), [lists/sets](/docs/feature-types#lists-and-sets), and [`dataclasses`](/docs/feature-types#dataclass).
...
The primary source of data for most companies are SQL databases (note, by SQL database we don't mean relational databases: we mean any database that can be queried with SQL—in practice this means pretty much any of them).
...
Our PostgreSQL database is the primary database used in our codebase, and our Snowflake database is used for analytics, with tables populated from DBT views and batch jobs.
...
To add your data sources to the Chalk dashboard, go to the `Data Sources` tab of the dashboard and click on the `Add a data source` button.
...
Keep track of the names that you've given to each of your data sources: these will be used in the next step. We recommend naming your `PostgreSQL` data source `postgres` and your `Snowflake` data source `snowflake`.
...
You will also need to add [metadata](/docs/sql#sql-file-resolvers) to the top of the file to tell Chalk how to ingest the data.
...
The `source` key (`-- source: postgres`) tells Chalk which [integration](/docs/integrations) to use to connect to the database.

Other comments in the SQL file resolver are indexed by Chalk and can be searched in the [Chalk dashboard](https://chalk.ai/projects).
...
For example, to fetch all transactions for a user, Chalk will modify the [get_transaction resolver](https://github.com/chalk-ai/fraud-example/blob/main/src/transaction.chalk.sql) to include a `where` clause for the user's `account_id`:
...
Chalk will automatically retrieve `User.account_id` and `User.name` with our [`get_user.chalk.sql`](https://github.com/chalk-ai/fraud-example/blob/main/src/user.chalk.sql) resolver.
...
In testing, new features, we recommend deploying your feature pipeline to a [branch](/cli/apply), which allows you to test your changes without affecting your production feature pipelines.
...
If you want to learn more, we also provide [additional docs on testing resolvers](/docs/unit-tests).

---

## Reverse ETL

Let's say we have a rather complex feature that we can't serve in realtime because it increases the latency of a query too much for a particular production use case.
For instance, lets say we want to add a new feature to our users that indicates the average time between their transactions. Maybe we believe that we can use this to detect whether a new transaction is fraudulent.
...
3. Load the new `average_time_between_transactions_30d` values into the Chalk online store.


Of note, we don't want to use our PostgreSQL database for this, we want to use Snowflake, which is optimized for bulk data loads unlike Postgres.
...
We can have Chalk [reverse-ETL](/docs/reverse-etl) our offline data into our online store by setting the [`max_staleness`](/api-docs#features.max_staleness) on a feature and using scheduled queries.
...
The [`max_staleness`](/docs/feature-caching) keyword argument tells Chalk how stale a feature value can get before it should be refreshed.
...
```sql src/get_user_offline.chalk.sql
...
Let's add a feature that computes the user's FICO score from our credit scoring vendor, [Experian](https://experian.com).
...
We are adding [strict validation](/docs/validation) to our `fico_score` feature to ensure that we only store and utilize valid FICO scores.
...
We use the [`Features`](/api-docs#Features) type to indicate which feature we expect to return.
Also note that we are initializing the `User` class with only the features that we want to update.
This partial initialization is the primary difference between Python's [`@dataclass`](https://docs.python.org/3/library/dataclasses.html) and Chalk's [`@features`](/api-docs#features).
...
As a sanity check, it can be helpful to use the [Chalk CLI](/cli/) to query a well-known input and ensure that we get the expected output.

We can use the [`chalk query`](/cli/query) command, passing in the id of a user, and the names of the features we want to resolve:
...
To help with this, Chalk can [codegen](/cli/codegen) a library of strongly-typed feature names for you.
...
You can see the generated code in [`clients/python/client.py`](https://github.com/chalk-ai/fraud-example/blob/main/clients/python/client.py).

If you are generating Python into a subdirectory of your Chalk project, be sure to add an entry to your [`.chalkignore`](https://github.com/chalk-ai/fraud-example/blob/main/.chalkignore#L14-L17) containing the directory of your generated code (in the above example, [`clients/`](https://github.com/chalk-ai/fraud-example/blob/main/clients/)).
