...
must be set to `load-balancer` in order to get a dns name that will route into the cluster. Routing can then be done in one
two ways:
1. Set `internal` to `false`, so that the clusters can route together over the open internet. For this to work, you
   must have public subnets attached to your VPC and an internet gateway et.
...
<Attribute field={'internal'} kind={'bool'}>
    Whether the connection pool load balancer attaches to interal or external subnets. Not usede if the service type is "cluster-ip".
</Attribute>
</AttributeTable>
