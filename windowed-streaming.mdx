...
When using tumbling windows, you can configure the size of the time intervals with the
[materialization](/api-docs#windowed.materialization) parameter. Consider the following code:
...
class User:
    ...
    total_transaction_amount: Windowed[int] = windowed(
        "30d",
        "90d",
        materialization={"bucket_duration": "5d"},
        expression=_.transactions[_.amount].sum(),
    )
```

This code creates windowed features representing the total amount of money the user has spent in the last 30 days and 90
days.

, the bucket duration is set to 5 days. Chalk will aggregate

In contrast, if the `materialization` parameter is not passed in, BLAH BLAH happens.
