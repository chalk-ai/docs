...
[Windowed features](/docs/aggregations) represent aggregated data pertaining to discrete windows.
...
- The resolver `user_stream_source` returns `User.id` and `User.name`, and neither of these features are related to the
...
- The resolver `transaction_stream_source` returns `Transaction.id`, `Transaction.user_id`, and `Transaction.amount`.
