...
a `dataset_name` in an offline query creates versioned revisions of [Datasets](/docs/datasets), including 
...
as a [Scheduled Query](/docs/scheduled-query). 
...
[Offline queries](/api-docs#ChalkClient.offline_query) support multiple input formats.
For most workflows, we recommend using Chalk SQL with `input_sql`.
Regardless of format, the primary key feature must be included in the input.

#### Recommended: Chalk SQL as input
`offline_query` can take [input_sql](/api-docs#ChalkClient.offline_query.input_sql) as an input
parameter, using Chalk SQL to construct the query spine directly from your offline data sources.

`input_sql` is recommended in the following scenarios:
...
`input_sql` can return features from multiple feature namespaces as columns of the result.
Each column must either correspond to an existing feature or be included in the
[output](/api-docs#ChalkClient.offline_query.output) list. For each referenced feature
namespace, that namespace's primary key must be included as a column.

The `__ts__` column is interpreted as the query execution time for each row. See our
documentation on [temporal consistency](/docs/temporal-consistency) for more details.
...
If needed, you can pass [input](/api-docs#ChalkClient.offline_query.input) directly as:

- [chalk.DataFrame](/docs/dataframe)
- [pandas.DataFrame](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html)
- a mapping from features to value lists
...
For `input_sql`, include a `__ts__` column in the SQL to set row-level query time.
For DataFrame or mapping inputs, pass [input_times](/api-docs#ChalkClient.offline_query.input_times).
...
that you can specify that follows similar bucketing logic to [Materialized Windowed Aggregations](/docs/materialized_aggregations)
