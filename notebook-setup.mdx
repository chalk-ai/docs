---
title: Setup
description: Setup Chalk in your notebook of choice
published: true
---
...
We will need the `CLIENT_ID` and `SECRET_KEY` of the access token we generated to initialize a Chalk client in our notebook.
...
If you are working with a local notebook, add the following to your `.env` file

```bash
CHALK_CLIENT_ID=<CLIENT_ID>
CHALK_CLIENT_SECRET=<CLIENT_SECRET>
```

and then initialize the Chalk client in your notebook as follows:
...
client= ChalkClient(
    ...
    client_secret=os.getenv("CHALK_SECRET_KEY"),
)
```
...
client.create_branch(BRANCH_NAME, switch=True)
```
