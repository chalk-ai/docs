...
Using the `has_many` function and the `isnear` method, we can express a relationship where we want the nearest documents for each query.
...
class SearchQuery:
  ...
  nearest_documents: DataFrame[FAQDocument] = has_many(lambda: SearchQuery.embedding.isnear(FAQDocument.embedding))
  response: str
...
class SearchQuery:
  nearest_documents: DataFrame[Document] = has_many(
    lambda: SearchQuery.embedding.isnear(
...
class SearchQuery:
  nearest_documents: DataFrame[Document] = has_many(
    lambda: SearchQuery.embedding.isnear(FAQDocument.embedding, index=None)
  )
```
