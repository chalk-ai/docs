...
<TipInfo>
  ...
  in the Primary key of the feature. For example, for the <code>has-one</code> join below, you could create a SQL resolver that resolves <code>PrequelLink</code>
  by running a query like: <code>SELECT id, prequel_id FROM books</code>.
</TipInfo>
