...
the `store_plan_stages` flag, which stores the outputs of each of the plan stages,
which will be visible in the web dashboard's query detail view.
More on these and other tips [here](/docs/debugging-queries).

### Running large offline queries

Offline queries run on a large number of rows require their own compute resources and infrastructure
to ensure they complete and do not contend with resources from other workloads like the engine or branch server.
Chalk handles this by launching a Kubernetes job that can runs asynchronously on dedicated compute nodes.
...
If you'd like to adjust the resource compute or memory for the job,
...
First, check the offline query dashboard: you'll be able to download individual or all shards of the output there.
