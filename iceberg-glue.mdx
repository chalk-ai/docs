...
```python
def scan_iceberg(target: str, catalog: BaseCatalog, columns: Sequence[str], custom_partitions: Mapping[str, tuple["date_trunc(day)", str]] = {}):
```
...
`custom_partitions` can be used to alter default Iceberg partition pruning behavior: in the example above, the Glue catalog table
is partitioned by `transaction_date`, but Chalk's timestamp column is `transaction_timestamp`. The `custom_partitions` argument
is used to specify that the `transaction_date` partition should be derived from the `transaction_timestamp` column using
the `date_trunc(day)` transformation. If the underlying Iceberg dataset were partitioned by `transaction_timestamp` instead,
the `custom_partitions` argument would not be necessary.
