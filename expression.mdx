...
<TipBadText
    header={
        <div>
            Do not use Python's <code>and</code>, <code>or</code>, <code>not</code>, or <code>is</code> operators
            in expressions.
        </div>
    }
>
    Python does not allow these operators to be overridden, so they will not work with Chalk's expressions.
    Instead, use the infix operators <code>&</code>, <code>|</code>, and <code>~</code> for boolean logic,
    and use <code>==</code> and <code>!=</code> for equality comparisons.
</TipBadText>
...
<TipGood>Use expressions in feature classes</TipGood>
...
<TipGoodText header={"Use materialized aggregations"}>
