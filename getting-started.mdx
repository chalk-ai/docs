...
```
2. Deploy your code. Pass the [`--no-promote`](/cli/apply#no-promote)
flag so that you don't affect the "actual" environment, and the
[`--await`](/cli/apply#await) flag so that your CLI will
wait for deploy to finish before returning. Take note of
the deployment ID so that you can use it when querying.
The deployment ID is the last part of the deployment url
returned by CLI.
```bash Terminal
chalk apply --await --no-promote
```
...
```
3. You can use a [SQL File Resolver](https://docs.chalk.ai/docs/sql#sql-file-resolvers)
...
```
4. After deploying (remember to use `--no-promote`),
...
### Add experian credit score
Now that you've got some experience under your belt, let's try
a challenge. In this exercise, we'll give you a new API to
integrate as a feature on `User`, with a new resolver.
This feature is another credit score (this time from Experian).
Add a second new feature that is the average of the
`rutter_score` and the `experian_score`.
Make sure to deploy with [`--no-promote`](/cli/apply#no-promote)
and query for your new feature using [Jupyter](https://jupyter.org/). Good luck!

Here's an example using the API:

```bash Terminal
$ curl -X POST https://experian.chalk.dev/bureau_score \
       -H 'Content-Type: application/json' \
       -d '{"name": "Riley Siebel", "email": "riley@chalk.ai"}'
```

If you do this right, you should be able to run the below query, even though the experian API takes an email as input. How do you think that works!?

```python Jupyter
client.query(input={User.id: 'u_a8fU4zT0Y8'}, output=[User.experian_score])
```

### Combine 2 features

Now create another feature to average the 2 credit scores. The user with id: 'u_a8fU4zT0Y8' should have an average credit score of ~18.8.
