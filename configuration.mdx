...
For your `@before_all` functions, if there is no `venv` specified, the function will run for actors in every
virtual environment. Otherwise, if there is a `venv` specified, the function will only run for actors in the specified
...
# this will run for all actors in all venvs
@before_all()
def before_all_hook():
...
# runs for actors **only** in specified venv
...
def get_user_risk_score_1(
    ...
    return model_1.predict(id, email_domain, recent_activity_count)
...
def get_user_risk_score_2(
    ...
    return model_2.predict(id, email_domain, recent_activity_count)
