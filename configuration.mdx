...
def get_user_risk_score_1(
    ...
    weights = msvc_client.get_weights("model-1")
    return model_1.predict(id, email_domain, recent_activity_count, weights)
...
def get_user_risk_score_2(
    ...
    weights = msvc_client.get_weights("model-2")
    return model_2.predict(id, email_domain, recent_activity_count, weights)
```
