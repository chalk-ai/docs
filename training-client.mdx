...
[resolvers that are marked as offline](/docs/resolver-offline-online). The default
behavior for offline queries is to only read ouputs from the offline store.

In an offline query you can request multiple instances of a feature set—additionally,
by default, an offline query returns all matches for a primary key in the offline store.
For instance, if you requested the `num_transactions` feature for users with id's
`1`, `2`, or `3`, you could receive multiple rows for each user if you've computed
the `num_transactions` feature for each user multiple times.

The main two use cases for offline queries are: assembling datasets and running batch jobs.

Chalk supports a number of client that can run offline queries. In this section, our examples
focus on our python client, which integrates nicely with jupyter notebooks.

<JupyterNotebook />

## How do Offline Queries Work?
