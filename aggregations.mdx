...
Here's an example where we track a cardholder's spend in the last 30 and 90 days, grouped by `mcc` (merchant category
code):
...
The grouped windowed feature's type must be [`DataFrame`](/api-docs#DataFrame). To reference a specific grouped value, 
...
Series shows the unique ID of the timeseries data underlying our aggregation system. Each
[namespace](/docs/features#namespacing), group, and agg (value to aggregate) represents a unique
timeseries. When possible, Chalk will use the same timeseries data to serve multiple features.
