...
First, we define a `Transaction` FeatureSet with a has-one relationships to the `User` FeatureSet. Within the `User`
FeatureSet, we will track a grouped windowed feature representing the user's total spend at each merchant over the past
30 days and 90 days.
...
In this example, `expression` groups all of the given user's `transactions` by each transaction's `merchant_id` value,
then aggregates each group by the sum of the transactions' `amount` values.

The User FeatureSet uses [`materialization`](/docs/aggregations#windowed-aggregation-for-performance) to aggregate the
transaction data as a performance optimization.

Finally, to query for spend at a specific merchant, we create a `HistoricalSpend` FeatureSet to combine the relevant
features:
...
class HistoricalSpend:
    ...
    user_id: "User.id"
    user: User
    spend_at_merchant_30d: float = _.user.spend_by_merchant["30d", _.merchant_id == __.merchant_id] # syntax TBD
    spend_at_merchant_90d: float = _.user.spend_by_merchant["90d", _.merchant_id == __.merchant_id]
```
...
of [namespace](/docs/features#namespacing), group (see
[`group_by_windowed`](/docs/aggregations#grouping-windowed-features)), and agg (value to aggregate) columns represents a
separate timeseries. When possible, Chalk will use the same timeseries data to serve multiple features.
