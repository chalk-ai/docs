...
A windowed feature can be referenced in a query or a resolver in the following, equivalent ways:

```py
# Note: The last value for each list is the time converted to seconds
User.num_failed_logins("**10m**") == User.num_failed_logins_10m == User.num_failed_logins__10m__ == User.num_failed_logins__600__
User.num_failed_logins("**1d**") == User.num_failed_logins_1d == User.num_failed_logins__1d__ == User.num_failed_logins__86400__
User.num_failed_logins("**1h30m**") == User.num_failed_logins_1h30m == User.num_failed_logins__1h30m__ = =User.num_failed_logins__5400__
```
