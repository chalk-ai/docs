...
|`approx_top_k` | An approximation of the most common values in a field. Takes in a required `k` parameter, such as `approx_top_k(k=5)` for approximately the five most common values. |
...
class Document:
    ...
    most_revised_documents: list[int] = _.revisions[_.document_id].approx_top_k(k=5)
...
class User:
    ...
    most_common_statuses: Windowed[list[str]] = windowed(
        "1h",
        max_staleness="15m",
        expression=_.login_attempts[_.status].approx_top_k(k=5)
    )
```
