...
| `mean` | Feature type must be `Vector`, `float` or <code>float &#124; None</code>. `None` values are skipped, meaning they are not included in the mean calculation.  |
|`count` | |
...
A [`windowed`](/api-docs/#windowed) feature takes in a series of window durations, and an aggregation expression
...
the current window's time range. The `_.chalk_now` keyword references the [query time](/docs/time#query-time) which
...
- by [materialized aggregates](/docs/materialized_aggregations).
