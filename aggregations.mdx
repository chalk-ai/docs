...
[`etl_offline_to_online`](/api-docs#windowed.etl_offline_to_online) to allow the features to be sent to the online store and
offline store after each window period. Windowed features often use [`default`](/api-docs#windowed.default) to set a
default value to return. Windowed features are typically resolved inline by expressions or
by a stream resolver.
