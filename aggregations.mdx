...
The User FeatureSet uses [`materialization`](/docs/aggregations#windowed-aggregation-for-performance) to aggregate
...
class HistoricalSpend:
    ...
    user_id: "User.id"
    user: User
    spend_at_merchant: Windowed[int] = windowed(
        "30d",
        "90d",
        expression=_.user.spend_by_merchant.group(merchant_id=_.merchant_id),
    )
```
