...
<Attribute field={'inputs'} kind={'map[string, JSON]'}>
  ...
  are provided by fully qualified path, rooted in the same feature namespace.  <a href={'/docs/has-many'}>Has many features</a>
...
<Attribute field={'query_name'} kind={'string?'}>
  The query name.  Information about <a href={'/docs/metricmonitor'}>Named Queries</a> is available in the dashboard.
</Attribute>
