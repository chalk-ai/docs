...
Typically, the Chalk team deploys the Data Plane into new cloud accounts that are owned and controlled by the customer. The Metadata Plane is owned and centrally managed by Chalk: it retains no customer application data, and is responsible for orchestration and configuration management.
...
- **An Online Store**: caches features with explicit staleness to accelerate feature retrieval. We provide a [number of different configuration options for the online store](#).
- **An Offline Store**: handles large datasets (both created and ingested by Chalk engines), and query logging. We provide a [number of different configuration options for the offline store](#).
