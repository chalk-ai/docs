...
class User:
    ...
    risk_score: float = feature(versions={
        ...
        2: feature(description="Version 1.0.0", expression=F.inference(risk_model_v1_0_0, _.age, 10, _.income))
    })
```
...
churn_model_latest = ModelReference.from(
    name="ChurnModel",
    alias="latest"
)
...
def load_model_artifacts():
    ...
    fast_tokenizer = PreTrainedTokenizerFast.from_pretrained(
        churn_model_latest.additional_files["tokenizer.json"]
    )
    churn_model_latest.load_model()

@online(resource_group="risk-model", resource_hint="gpu", model=churn_model_latest)
def embed_transaction_memo(memo: Transaction.memo) -> Transaction.memo_embedding:
    ...
    return churn_model_latest.predict(tokens)
```
