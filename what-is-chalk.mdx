...
metaDescription: 'Chalk is a data platform for inference that enables innovative machine learning teams to focus on building the unique products and models that make their business stand out.'
titleHidden: true
hideToc: true
published: true
---
...
Chalk lets you define features using Pythonic dataclasses–just like Pydantic–no DSLs.
Every feature is:
- typed and validated
...
class User:
    ...
    average_discount_percentage: float = _.txns[_.discount_percentage > 0].mean()
...
At inference time, query plans tailored to your input/output schema are generated dynamically by slicing this DAG into sub-graphs--ensuring that only the features needed are fetched--optimizing for speed, precision, and cost.

Note: We build a DAG instead of a tree to prevent redundant feature computations.

Each node in the DAG is a Chalk expression or resolver (SQL / Python):

- lambda like Chalk expression e.g. `is_expensive_purchase: bool = _.amount > 100`
- SQL resolver that connects to an underlying data store like Athena
...
Resolvers make it easy to integrate a wide variety of data sources, join them together, and use them in inference, while Chalk abstracts away dependency resolution and low-latency model execution.

Decoupling feature logic and model execution from underlying ETL results in

- faster iteration without pipeline rebuilds
...
@online
def get_username(
    email: User.email, # use type annotation to tell Chalk what feature to transform
) -> User.username: # the resulting feature is saved as username
...
Once we've defined our pipelines, deploy features to a branch in < 30s with:

```shell
chalk apply --branch=elvis
```

We can then query for features against this branch and perform QA workflows on our preview environment to make sure that our Chalk deployment performs as expected:
