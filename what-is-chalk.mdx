...
- Serving fresh features on-the-fly with versioning, branching, and full observability
    ...
    - A/B testing models and LLM prompts with real historical traffic
...
class User:
    ...
    # computed with `get_username`
    # a Python resolver (function)
    username: str
...
- A Python function that explicitly inputs and outputs features
    - Use the Python packages you're familiar with and easily call 3rd-party clients e.g. Polars, httpx, OpenAI, etc.
...
The `get_username` Python resolver below explicitly defines input dependencies through `User.email` and specifies the output feature type as `User.username`:
...
A successful execution of the `chalk apply --branch` command creates a new isolated branch (in this case, 'fraud-model-v2').
Chalk scans through resolvers, lints features, and deploys them to this separate branch.
This isolation enables testing changes independently before merging them into the production environment.
...
With this low-latency execution, Chalk seamlessly unifies offline and online inference — bridging the gap between experimentation and production.
This unified approach enables teams to:

- Establish a single source of truth for feature definitions and transformations that's shareable across the organization
...
Chalk offers the developer-friendly experience of Python while achieving high performance by transpiling functions like `get_username` into optimized Velox expressions using a Symbolic Python Interpreter.
...
**Observability:** See everything, debug anything
...
Deploy faster and with confidence — knowing exactly how your systems behave at each step of the machine learning lifecycle.
