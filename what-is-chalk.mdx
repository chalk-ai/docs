...
Handling the interplay between an increasingly complex mix of data sources and making transformed features
production-grade—that is making them consistent, debuggable, monitorable, and quick—is a hard problem,
but failing to address causes a lot problems:
- high query latency, (?)[link to how chalk addresses this problem]
- slow model deployment (?)[link to how chalk addresses this problem]
- inefficient and excessive recomputation of expensive features  (?)[link to how chalk addresses this problem]
- diverging implementations of feature processing code, unnoticed feature drift, (?)[link to how chalk addresses this problem]
- high storage overhead, (?)[link to how chalk addresses this problem]
- unrecoverable datasets based on historic data, (?)[link to how chalk addresses this problem]
- lack of feature versioning,  (?)[link to how chalk addresses this problem]
- difficultly spotting model performance degradation. (?)[link to how chalk addresses this problem]

The abstraction that is typically used to "productionize features" is a Feature Store: we will elaborate
later on [what Feature Stores are](what is a feature store) and [how Chalk is implemented](how chalk works), but
at a high level Chalk, does two things: 1). it centralizes and remembers every feature you've ever computed, and it
...
You can think of this as Version Control and Continuous Integration for your data.
