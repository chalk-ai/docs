...
Companies are increasingly built not only on data, but on data transformation. Machine Learning models
...
metrics and features on the fly—for their users or for monitoring purposes (maybe an example here).

This is a incredibly challenging problem. 1). tremendous amount of data,  2). running feature pipelines can be
an expensive operation and doing it in real time is hard, 3). engineered features are often treated
as temporary values and are discarded after creation—which is costly, makes it hard to debug errors,
increases the surface space for potential errors,
