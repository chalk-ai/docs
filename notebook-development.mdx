...
To install dependencies, run the `!pip install "chalkpy[runtime]"` bash command directly in a cell (using `!`). Alternatively,
you can add the `chalkpy[runtime]` dependency to your kernel's virtual environment.
...
To do this, open the Chalk Dashboard. Under **Settings > Access Tokens**, generate a
client ID and secret key pair by clicking on **New Token** and selecting all necessary permissions.
