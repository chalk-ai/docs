...
- Authenticating to Chalk (using the `chalk` cli or environment variables)

We provide specific setup instructions for [Deepnote](/docs/notebook-development#deepnote), [Hex](/docs/notebook-development#hex) and [Vertex](/docs/notebook-development#vertex-and-google-collab-enterprise) but the process should be similar for other notebooks environments.

## Installing the Chalk Python Dependency

To install dependencies, run the `!pip install "chalkpy[runtime]"` bash command directly in a cell (using `!`)
...
You can use these access credentials to connect Chalk to any Jupyter notebook environment—we walk through
...
If everything is properly setup, you should see your `user_id`, `team_id` and `environment_id` come back.

## Loading Features

With everything configured, we can start building some new features!
...
You can now use these features to build new features! Before getting started you'll want to import `_` from
...
To query for your new features, pass them as outputs to the `ChalkClient` query methods. For instance, to run an online query for your new `Director.exclaimed_name` feature, you can run the following:
...
Running the code above creates a Pandas `DataFrame` with the columns `Director.id`, `Director.name`, and `Director.name_exclaimed` (alongside some metadata columns). This `DataFrame` will have one row for your Directors with ids 0 to 999.
...
For instance, if we want to define a new feature on our Movie class that turns or str genres
...
2. Your features translate really easily into really performant code—everything we've written in this tutorial can be translated to your Chalk feature store and run performantly!
