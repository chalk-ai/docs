...
Whether you're building a content platform, a multi-sided marketplace, or the next FinTech, giving your users the best recommendations and in a timely fashion can make or break customer satisfaction and revenue.
...
Our marketplace has a few core entities and many interdependent relationships (primarily through interactions between users and sellers over a particular item):
...
Each set of features is decorated with `@features` (imported from [Chalk's SDK](https://docs.chalk.ai/api-docs)).

Note: Keyword arguments can be passed into our `@features` decorator to define metadata like [data ownership, tags, cache-policies, etc.](https://docs.chalk.ai/api-docs#features)
...
Whenever we request features for a User, with a command like: `chalk query --in user.id=50 --out full_name`;
Chalk knows to first fetch the `first_name` and `last_name` from the underlying data source, then compute `full_name` using the provided [expression](https://docs.chalk.ai/docs/expression).

This is all handled automatically, Chalk at it's core is a feature orchestration engine that can dynamically build query plans to fetch and compute features on-demand. 

In otherwards, requesting `chalk query --in user.id=50 --out first_name` would produce a different query plan than when we requested just the `full_name`.
