...
In other words, requesting `first_name` instead of `full_name` with `chalk query --in user.id=50 --out first_name` produces a totally different query plan than when we requested just `full_name`.
...
The return type is also explicitly hinted as `Review.normalized_rating`.
Setting up the type hints in this way enables us to (necessarily) build the dependency graph of your features (that we leverage for query planning) while preserving as much developer experience as possible--we want to minimize boilerplate and the cognitive load required to express complicated feature logic.
...
Whenever we want to explicitly reference a specific window within a Python resolver, we can index the `Windowed` feature like so: `User.review_count["30d"]`.
...
Computing heavy aggregations at high QPS can be extremely load-bearing on your underlying data infrastructure, especially since real-time use-cases (by definition) necessitate fetching features directly from your raw data sources, for maximum freshness.
...
We'll leverage this to get a sense of whether this item is currently discounted, the most recent price (across all sellers), and how many times it's been purchased (which we can easily make windowed).

Notice how we can filter the DataFrame with `_.type == "purchase"` before applying the aggregation function to guarantee we're only counting actual purchase events.
