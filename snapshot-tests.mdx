...
In this example we'll be testing that snapshot testing the features on the following feature class:
...
In this example, lets say we resolve the `full_name` and birthday of our User's from from a postgres database. The age, we calculate using an online resolver:
```python
...
Though this is a pretty trivial example, lets set some examples on how we expect the data to drift between invocations:
- We expect a User's birthday to never change,
- We expect User's full_name to change in a maximum of 0.001% of our users (1/100,000),
- We expect age to change by a maximum amount of 30 days (in reality average age will increase by exactly the amount of times between invocations of our snapshot test).
