...
def snapshots(client: ChalkClient):
    ...
    # The snapshot will find the dataset associated with the last merged snapshot
...
GitHub Action is set to run on `pull`, we automatically have access to not only the
