...
These fixtures serve to output two Pandas DataFrames: the main snapshot (which we store in
a dataset called `user_snapshot`) and the branch snapshot (which is generated
...
script will update the `user_snapshot` dataset with the latest branch code.
...
will run when we merge into our main branch.
...
The second GitHub Action runs our update script to update the `user_snapshot` dataset
with our latest snapshot.
  ...
  update-snapshot:
    steps:
      ...
      - name: Update snapshot
        run: |
            python3 scripts/update_user_snapshot.py
```
...
You can do this by running an offline query using the python client to create the first snapshot:
