...
Offline, event tables can be ingested
incrementally.
...
Here, the `${user.id}` variable defines the inputs of the query, and `user_table`
matches the PostgreSQL table name. The comments are `yaml`-parsed to provide other metadata for
Chalk to decide how to design the resolver.

SQL file resolvers can also return multiple rows for aggregation operations, offline queries, and more.
Notice below the omissions of the `count` argument and the `where` clause of the SQL query.

```sql get_users.chalk.sql
-- type: online
-- resolves: user
-- source: PG
...
Supported sources are <code className="dark:text-white">['snowflake', 'postgres', 'postgresql', 'mysql', 'bigquery', 'cloudsql', 'redshift', 'sqlite', 'kafka']</code>.
</Attribute>
...
Parameters for incremental queries. For more information, see the <a href='/docs/sql#incremental-queries'>below</a> section on incremental queries.
...
The length of the window from the last observed row that Chalk will re-ingest, e.g. <code className="dark:text-white">1h</code>.
</SubAttribute>
...
Must be supplied in <code className="dark:text-white">row</code> and <code className="dark:text-white">group</code> modes.
</SubAttribute>

</SubAttributeTable>
</Attribute>

<Attribute field='count' kind='"1" | null'>
...
See [Duration](/api-docs#Duration) for more details.
</Attribute>
...
we can map the <code className="dark:text-white">arbitrary_column_name</code> to a Chalk feature belonging to the
