...
class MyFeatures:
    ...
    date1: date
    date2: date
    date_diff_days: float = F.total_seconds(_.date2 - _.date1) / (60 * 60 * 24)
    date_diff_weeks: float = F.total_seconds(_.date2 - _.date1) / (60 * 60 * 24 * 7)
```
...
online store. If you set `cache_nulls=False`, null values will not be persisted in the online store.
...
class MyFeatures:
    id: Primary[str]
    feature1: float = feature(cache_nulls=False) # null values will not be persisted
    feature2: float = feature(cache_nulls=True) # null values will be persisted. This is the default behavior.
```
...
Additionally, the feature table in the dashboard has been updated to allow for selection of visible columns, which
enables viewing request counts over multiple time ranges in the same view.
