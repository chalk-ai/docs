...
You can now add tags to your deployments. Tags are unique on your environments. If you add an already existing tag to a new deployment, Chalk will remove the tag from the old deployment.
...
You also can specify a required pattern for your tags by adding a `tagregex` field to your `chalk.yaml` file. Any deployment tags will need to match your supplied regex. For instance:
