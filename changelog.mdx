...
parameters to allow for more granular control over the parallelization of your query execution. To see all of your
offline query options, check out the [offline query documentation](/api-docs#ChalkClient.offline_query).

In addition, offline query progress reporting now specifies progress by shard, to give developers more insight
into where their offline query is in the execution progress.

### ChalkClient can now use the default Github branch

You can now default to using the name of your current Github branch when developing using the `ChalkClient`.
...
You can now use `regexp_like`, `regeexp_extract`, and `regexp_extract_all` to do regular expression matching and
