...
@features
class User:
	composite_key: str = _.last_name + "-" + _.zipcode
	posts: DataFrame[Posts] = has_many(lambda: User.composite_key == Post.user_code)
	profile: Profile = has_one(lambda: (Profile.user_email == User.email) & (Profile.user_id == User.id))
```
