...
[Query planning](https://docs.chalk.ai/docs/query-online) is now even faster due to optimization work on the planner.
...
Logs and [tracing](https://docs.chalk.ai/docs/tracing) now run on ClickHouse, improving search performance. Facets—indexed attributes like service,
...
![Log Explorer Page with New Facets](/img/changelog-26_01_26-log-facets.png)

### Enhanced query plan visualization

The [query plan visualizer](https://docs.chalk.ai/docs/debugging-queries#query-plan-visualizer) now supports
...
![Log Explorer Page with New Facets](/img/changelog-26_01_26-query-plan-nav.png)


### Store plan stages for scheduled queries

[Scheduled queries](https://docs.chalk.ai/docs/scheduled-query) now include a "Store plan stages" feature.
...
![Example of stream message preview interface](/img/changelog-25_12_12-stream-message-previews.png)
...
![Optional new navigation pane layout example](/img/changelog-25_12_12-new-layout.png)
...
[Tracing](https://docs.chalk.ai/docs/tracing) now provides full visibility into how queries execute inside Chalk.
...
![Example Query Trace](/img/changelog-2025_11_25-query_trace.png)
...
Read more about Chalk’s [metrics monitoring](https://docs.chalk.ai/docs/metricmonitor).

![Stream Monitoring Incident Graph](/img/changelog-2025_10_27-stream_metric_incident.png)
...
Discover more about [Chalk DataFrame](https://docs.chalk.ai/docs/dataframe) aggregations and filters.
...
Learn more about [scheduling queries](https://docs.chalk.ai/docs/resolver-cron) for automated runs.

![Stream Monitoring Incident Graph](/img/changelog-2025_10_27-scheduled_query.png)
...
Learn more about environment visibility in our [docs](https://docs.chalk.ai/docs/resource-configuration#kubernetes).

![Kubernetes Service Accounts](/img/changelog-2025_10_23-service_accounts.png)
...
Added new (materialize-able) [aggregations](https://docs.chalk.ai/docs/materialized_aggregations) (min_by_n/max_by_n) for computing the smallest/largest n elements based on any specified ordering column.
...
![Connections pane](/img/changelog-2025_10_13-connections_pane.png)
...
[Dynamic expressions](https://docs.chalk.ai/docs/expression#dynamic-expressions) enable you to compute new features on-the-fly during online queries without needing to define them in advance.
...
Enhanced [Chalk's diff viewer](https://docs.chalk.ai/cli?command=diff) to show the entire file contents for added/removed files instead of just file names; helps track feature and resolver changes between deployments and branches.

### New Chalk expression for computing the top most common values for a field

Added a [new windowed aggregation](https://docs.chalk.ai/docs/aggregations#define-an-aggregation) (`approx_top_k`), for computing an approximation of the most common values in a field.
...
![Re-running Offline Queries](/img/changelog-2025_09_26-rerun-offline-query.png)
...
![Output explorer with has-one and has-many features](/img/changelog-25_07_30-has_one.png)
...
![Inputs explorer for offline queries](/img/changelog-input_explorer_for_offline_queries.png)
...
![Accelerated Python function](/img/changelog-2025_06_06-accelerated_function.png)
...
![All charts page](/img/changelog-2025_06_06-all_charts.png)
...
![Resource configuration](/img/changelog-25_05_30-resource_configuration.png)
...
[Feature versions](/docs/feature-versions) support [Chalk expressions](/docs/expression), allowing you to define different computation logic for each version of a feature.
...
We've published a [migration guide](https://github.com/chalk-ai/chalk-ts#migrating-to-the-grpc-client) for transitioning from the HTTP client to the new gRPC implementation in TypeScript.
...
Chalk can connect to your ClickHouse database and other data sources with [standard SQL syntax](/docs/sql).
...
We've refreshed our [introductory documentation](/docs/what-is-chalk) with more comprehensive examples and new sections that showcase how Chalk serves different roles (AI Engineers, Data Scientists, and Data Engineers).

### Case study with MoneyLion—a leading fintech for banking, investing, and credit building

With Chalk, [MoneyLion replaced fragmented Java microservices](https://chalk.ai/customers/moneylion) and manual rewrites with a unified Python-first feature platform.
...
We've published our [quarterly product update,](https://chalk.ai/blog/product-update-april-2025) which summarizes improvements and updates from this past Spring, including expanded Python-to-C++ compilation, selective feature persistence controls, and enhanced observability tools.

### April Events Roundup

We've published a recap of the various AI and data conferences our team attended in April 2025.
The [blog post](https://chalk.ai/blog/events-roundup-april-2025) covers our participation at NexGen Banking Summit, VeloxCon, Agents & GenAI Infrastructure Summit, OptimizedAI Conference, and Data Council, including key talks and insights we shared about building infra for real-time ML at scale.
...
[**Check out the talk (18 mins) on our YouTube page!**](https://www.youtube.com/watch?v=VsAuT1QjBDI)
...
![Offline query explorer](/img/changelog-25_04_21-offline_query_explorer.png)
...
Check out our [new engineering blog post](https://chalk.ai/blog/symbolic-python-interpreter) for a deep dive into how our Symbolic Python Interpreter works.
...
![Resolved by column](/img/changelog-25_04_04-resolved_by.png)

![Online Query Performance](/img/changelog-25_04_04-online_query_performance.png)
...
[With Chalk, Apartment List](https://chalk.ai/customers/apartment-list) delivers instant apartment recommendations with real-time price and location flexing, dynamically adapting to user behavior while making low-latency calls to third-party services for the most current pricing information.
...
![Sources in Chalk Data Explorer](/img/changelog-25_03_25-data_explorer.png)
...
Released [v1.2.0 of the Chalk Go Client](https://github.com/chalk-ai/chalk-go/releases/tag/v1.2.0) which introduces some breaking changes, most notably:

- The [gRPC implementation of Client](https://docs.chalk.ai/docs/query-overview#running-queries-using-g-rpc) is removed and replaced with a consolidated NewGRPCClient constructor--configuration remains mostly unchanged.
- `OnlineQueryBulk removed to prevent inefficient data conversions, migrate to `OnlineQueryBulk`.
  - Use `GRPCOnlineQueryBulkResult.UnmarshalInto` for structured responses.
...
![Static expressions](/img/changelog-25_03_14.png)
...
With Chalk, Verisoul [iterates on fraud signals,](https://chalk.ai/customers/verisoul) deploys new updates in hours, and increases detection accuracy.

### Easily import feature classes with Chalk Client when experimenting locally

We have implemented two new functions for Chalk Client [to aid in local experimentation:](https://docs.chalk.ai/docs/notebook-development#loading-features) `client.load_features()` and `client.get_or_create_branch()`.
...
We've also updated our guide for [developing with Jupyter Notebooks](https://docs.chalk.ai/docs/notebook-development).

### Breaking changes to Chalk Client's Go SDK

Released [v1.0.0 of the Chalk Go Client](https://github.com/chalk-ai/chalk-go/releases/tag/v1.0.0) which introduces some breaking changes, most notably:
...
- The deployment metrics chart can now be filtered by [named (custom) tags](https://docs.chalk.ai/docs/changelog#june-28-2024)
- The Chalk dashboard has a new errors page for viewing exceptions and issues with Chalk queries

![Errors page](/img/changelog-25_02_27.png)
...
We call the results returned from these offline queries [Datasets](https://docs.chalk.ai/docs/datasets).
Chalk supports exporting Datasets back into your [AWS Glue catalog](https://docs.aws.amazon.com/prescriptive-guidance/latest/serverless-etl-aws-glue/aws-glue-data-catalog.html), letting other teams discover them and enabling downstream analytical workflows.
...
- The pages for [Named Queries](https://docs.chalk.ai/docs/named-query) and Resolvers [now also include errors,](https://docs.chalk.ai/docs/query-errors#chalkerror) use the "Logs and Errors" tab to switch to this filtered view
  - Click and drag your mouse within the time series graph to zoom in into a time interval and filter down the errors table

![Logs and Errors](/img/changelog-25_02_20-errors.png)
...
![Data Explorer](/img/changelog-25_02_13-data_explorer.png)
...
![Scheduled queries](/img/changelog-2025_02_06-scheduled_runs.png)
...
Chalk can automatically create embeddings for [vector feature types](/docs/feature-types#vectors) through Open AI.
We've expanded our native embed function to support [Vertex, Google Cloud's AI platform.](https://cloud.google.com/vertex-ai)
...
Extending the support for `Now` unlocks [new design patterns](/docs/python-resolvers#time-dependencies); build resolvers and queries that take in

- multiple ids at a single point in time
- a single id at various points in time
- multiple ids at various points in time

[Easily sample historical state](/docs/temporal-consistency) like all the transactions for 3 users from a particular date or include the current time when calling out to a microservice.
...
Chalk checks your features for errors locally before [deploying them to a remote branch.](/docs/branches)
...
### Breaking change in the 1.0.0 major release of [Chalk's Java client](https://github.com/chalk-ai/chalk-java)
...
![Profiling an offline query](/img/changelog-25_01_30-offline_query_performance.png)
...
![Feature metrics view](/img/changelog-25_01_15-feature_metrics.png)
...
We've implemented [**KEDA (Kubernetes-based Event Driven Autoscaler)**](https://keda.sh/) and are rolling it out to all customer clusters over the next few weeks.
...
Read more about how to view [logs using the Chalk CLI in our docs.](/cli/logs)
...
![Graph view for a data lineage](/img/changelog-datalineage.png)
...
![Updated log explorer](/img/log_search_filters.png)
...
![Source code viewer with file search](/img/view_source_file_explorer.png)

### Include query context in Chalk Client queries

The Chalk Client now offers a [query_context](/api-docs#ChalkClient.query.query_context) field that can be accessed
...
[Chalk expressions](/docs/expression). You can now use [`jaro_winkler_distance`](/api-docs#jaro_winkler_distance)
to compute the Jaro-Winkler distance between two strings, [`partial_ratio`](/api-docs#partial_ratio) to compute
the [fuzzywuzzy](https://pypi.org/project/fuzzywuzzy/) partial ratio between two strings, and
[`length`](/api-docs#length) to compute the length of a string. We've also added
[`struct_pack`](/api-docs#struct_pack) to construct a struct from a mapping, [`year`](/api-docs#year) to extract
the year from a date, and [`last_day_of_month`](/api-docs#last_day_of_month) to return the last date in the month
given a date. To read more about all of the `chalk.functions` available for writing expressions, check
out our [`API docs`](/api-docs#coalesce).
...
![Source code viewer with text search](/img/source-code-viewer-dec-2024.png)

### Dataset revision metadata get and set

[`DatasetRevision`](/api-docs#DatasetRevision) objects now have `get_metadata` and `set_metadata` methods to get and
...
methods in our [API documentation](/api-docs#DatasetRevision).

### Expressions support more array functions

The `chalk.functions` library now supports more array functions that can be used in [expressions](/docs/expression).
...
order. Read more about all of the `chalk.functions` available for writing expressions in our [`API docs`](/api-docs#section-Functions).
...
You can now reference dataclass struct fields with [expressions](/docs/expression).
...
in your toolbox for writing expressions, check out our [`API docs`](/api-docs#section-Functions).
...
![Overview page in dashboard](/img/overview-page-dec-2024.png)
...
![Stack trace for pods in the cluster](/img/stack-trace-dec-2024.png)
...
![Values preview for features in dashboard](/img/values-preview-dec-2024.png)
...
We've updated our documentation on [windowed aggregations](/docs/aggregations) to include more information on how to
...
You can now use [Poetry](https://python-poetry.org/) to manage Python dependencies in your Chalk project. Simply
set the `requirements` field in your `chalk.yaml` file to `pyproject.toml` to use Poetry for dependency management.
Read more about how to use configuration files [here](/docs/configuration).
...
We've added more functions to `chalk.functions` that can be used in [expressions](/docs/expression).
You can now use [json_extract_array](/api-docs#json_extract_array) to extract arrays of strings, bools, numbers, or
nulls from a JSON feature, [safe_divide](/api-docs#safe_divide) to divide two numbers safely, returning `None` if the
denominator is zero, [to_iso8601](/api-docs#to_iso8601) to convert a datetime feature to an ISO 8601 string,
[from_unix_seconds](/api-docs#from_unix_seconds) and [from_unix_milliseconds](/api-docs#from_unix_milliseconds) to
convert a Unix timestamp in seconds or milliseconds to a datetime feature, and finally
[regexp_replace](/api-docs#regexp_replace) to replace substrings in a string using a regular expression. To read more
about all of the `chalk.functions` in your toolbox for writing expressions, check out our
[`API docs`](/api-docs#section-Functions).
...
![Warning banners in dashboard](/img/warning-banners-dec-2024.png)
...
[Offline queries](/docs/query-offline) can now accept parquet files as input by passing in a `”s3://“` or `”gs://“`
...
To read more about offline query parameters, see our [API docs](https://docs.chalk.ai/api-docs#ChalkClient.offline_query).
...
[CLI docs](https://docs.chalk.ai/cli/aggregate_backfill).

### Expressions now support more array and encoding functions

We've added more functions to `chalk.functions` that can be used in [expressions](/docs/expression).
You can now use [`array_join`](/api-docs#array_join) to concatenate array elements into a string,
[`cardinality`](/api-docs#cardinality) to count the number of elements in an array, as well as
[`max`](/api-docs#max) and [`min`](/api-docs#min) to find the maximum and minimum values, respectively, in an array.
You can also use [`max_by`](/api-docs#max_by) and [`min_by`](/api-docs#min_by) to find the row with the maximum and
minimum values in a given column in a DataFrame. For mathematical operations, you can now [`round`](/api-docs#round)
numbers and use [`from_base`](/api-docs#from_base) for base conversion. For encoding, you can now compute
the [`sha1`](/api-docs#sha1), [`sha256`](/api-docs#sha256), and [`sha512`](/api-docs#sha512) hashes of strings.
Finally, you can use [`format_datetime`](/api-docs#format_datetime) to format datetime features using a format string
and [`strpos`](/api-docs#strpos) to find the first position of a substring in a string. To read more about all of the
...
you can now use the [`cache_defaults`](/api-docs#feature.cache_defaults) parameter to optionally cache default feature
...
[API docs](/api-docs#feature.cache_defaults).
...
You can now retrieve [Map document types](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.NamingRulesDataTypes.html#HowItWorks.DataTypes)
...
![Detailed Kubernetes logs in dashboard](/img/k8s-dashboard-logs-nov-2024.png)

### Improved stacktrace rendering in Chalk CLI

We've drastically improved the rendering of stacktraces in the [Chalk CLI](/cli/chalk) with improved syntax highlighting
...
expressions](/docs/expression). For example, you can now express the following features to aggregate
...
points can be used in [expressions](/docs/expression) to define features with code that can be
...
In addition to being able to make SageMaker calls, [expressions](/docs/expression) now support a
...
[Expressions](/docs/expression) now support datetime subtraction and the use of a new
...
We have a new offline query configuration option for recomputing features _only_ when they are not already available in
...
can be found in our
[expression documentation](/api-docs#section-Chalk%20Functions).
...
We added a new Aggregations page to the dashboard where you can see the results of [`aggregate backfill`](/cli/aggregate_backfill) commands. Check it out to see what resolvers were run for a backfill, the backfill's
...
aggregations](/docs/materialized_aggregations).
...
[Expressions](/docs/expression) on [windowed](/docs/aggregations) features can now include the special
...
We shipped static planning of [expressions](/docs/expression). expressions
