...
If your resolver depends on features from other feature classes (such as `User.ssn`), we would not recommend this
chaining pattern. Instead, we recommend falling back to the existing way of writing a has-one join between `User` and
`CreditReport`:
