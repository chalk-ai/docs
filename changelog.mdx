...
`feature` field and set [`cache_nulls`](/api-docs#feature.cache_nulls) to False.

### More static execution of certain Python resolvers

We built a way to statically interpret Python resolvers to identify ones that are eligible for static C++ execution.
For now, resolvers are eligible if they do simple arithmetic and logical expressions. If you're interested in learning
more and seeing whether these new query planner options would apply to your codebase, please reach out!
