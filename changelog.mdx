...
### Chalk's C++ SQL driver now speeds up parameterized Spanner queries
...
Expanding the types of events and their granularity provides better insights into issues and jobs.
These include node removal, pod scheduling, and deployment failures.

Fixed a bug where breadcrumbs did not correctly hyperlink to a feature's parent in the query plan viewer, enabling quicker traversal through a feature's lineage.
