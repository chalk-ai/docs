...
We’re excited to share that we’ve implemented **KEDA (Kubernetes-based Event Driven Autoscaler)** and are rolling it out over the next few weeks.
...
View logs using the Chalk CLI! Inspect your log stream the same way you would from the GUI but via the command line!
`chalk logs [--query="the same string you can put in the web ui"]`

## Export and view metrics using the CLI

View and export metrics using the Chalk CLI!
In addition to exporting metrics from with a custom OpenMetrics compatible collector, you can now also export metrics directly using the CLI.
`chalk metrics export`

## Visualize and trace your feature's data lineage with a Graph View

A new **graph view** in the web GUI--use the graph view to visualize the data lineage of your features and trace how your features are created and connected!
You can also track the named queries that reference each feature.
