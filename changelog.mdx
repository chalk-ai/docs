...
These descriptions can also be added to charts created from the web.
...
[SQL file resolvers](https://docs.chalk.ai/docs/sql#sql-file-resolvers) are Chalk's preferred method of resolving
...
[Stream resolvers](https://docs.chalk.ai/api-docs#stream) support a `keys=` parameter. This parameter allows you to re-key a stream by a property of the message,
...
in the "online query" API [response object](https://docs.chalk.ai/api-docs#ChalkClient.query).
...
Chalk [clients](https://docs.chalk.ai/api-docs#ChalkClient) can also be scoped to a branch by passing the branch in the constructor.
