...
displays the available keys for filtering, shown below.

![Updated log explorer](/img/log-search-filters.png)

### Search by file name in the source code viewer

In the Source Code viewer, found in the Deployments page of the dashboard, you can now search for files by name
in addition to the existing text search through the source code from the linked deployment.
