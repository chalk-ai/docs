...
You can now run SQL queries in the dashboard to explore data in [datasets](https://docs.chalk.ai/docs/datasets).
To use this feature, navigate to the `Datasets` page in the menu, select a dataset,
and click on the `Output Explorer` tab.

![SQL Explorere for Datasets](/img/sql_explorer_datasets.png)
...
### Set environment variables and more in Advanced section of Cloud Resource Configurations page in dashboard
...
you can now also set environment variables and other settings like Kubernetes Node Selectors. A notable
environment variable that you can set is `AUTO_SHUTDOWN_PERIOD`, which should be set to the number of seconds
of inactivity after which the branch server is automatically shut down. The Kubernetes Node Selector allows
you to specify the machine family you would like to use for your deployment. For example, this would be [EC2
Instance Types](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/instance-types.html) for AWS deployments or
[Compute Engine Machine Families](https://cloud.google.com/compute/docs/machine-resource) for GCP deployments.
If you have any questions about how to use any of these settings in the configuration page, please reach out
to the team.

![Cloud Resource Configurations Advanced Settings](/img/cloud_resource_configuration_advanced_10_2024)
...
documentation](/docs/training-client#sql-queries-as-input).
