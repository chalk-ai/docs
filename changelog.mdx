...
### Query Trace available for offline queries

When you run an offline query with `store_plan_stages=True`, you can now download the query trace with
information on the timing and performance of each node in the query plan. To download the query trace,
navigate to the offline query page in the dashboard, click the `Download Full Dataset` button, and select
`Query Trace`.

![Offline Query Download Query Trace](/img/download_query_trace_nov_2024.png)
