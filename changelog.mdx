...
### Chalk's C++ SQL driver now supports Spanner DB (GCP)

Chalk’s C++ driver is steadily advancing towards SQLAlchemy parity, with the notable addition of formal Spanner support.
Spanner queries now run faster, with some query latencies reducing from 24ms to 10ms.

### Support for composite (primary) keys with Chalk expressions

Added support for defining composite keys with Chalk expressions!
This works with basic resolvers, parent-child relationships, and DataFrame resolvers.
Chalk automatically computes primary keys for stores requiring single-key inputs, opening up new ways to dynamically join data!

```python
@feature
class Student:
    id: str = _.first_initial + _.last_name + _.class_year
    first_initial: str
    last_name: str
    class_year: str
```
...
The Chalk query plan viewer now colors SQL resolver nodes and Chalk expression nodes as orange.
...
Expanding the types of events and their granularity increases visibility into issues and jobs like node removal, pod scheduling, and deployment failures.

Fixed a bug with breadcrumbs not correctly hyperlinking to a feature’s parent (feature) within the query plan viewer, enabling quicker traversal through a feature’s lineage.
...
given a date. To read more about all of the `chalk.functions` available for writing underscore expressions, check
...
### Underscore expressions support more array functions

The `chalk.functions` library now supports more array functions that can be used in [underscore expressions](/docs/expression).
...
order. Read more about all of the `chalk.functions` available for writing underscore expressions in our [`API docs`](/api-docs#section-Functions).

---

## December 9, 2024

### Underscore expressions support struct field access and more functions

You can now reference dataclass struct fields with [underscore expressions](/docs/expression).
We've also added more functions to `chalk.functions` that can be used in underscore expressions,
...
in your toolbox for writing underscore expressions, check out our [`API docs`](/api-docs#section-Functions).
...
### Underscore expressions now support more datetime, JSON, string, and math functions

We've added more functions to `chalk.functions` that can be used in [underscore expressions](/docs/expression).
...
about all of the `chalk.functions` in your toolbox for writing underscore expressions, check out our
...
### Underscore expressions now support more array and encoding functions
...
`chalk.functions` in your toolbox for writing underscore expressions, check out our
[`API docs`](/api-docs#section-Functions).
...
You can now define features as map types and reference these map features in underscore expressions.
...
points can be used in [underscore expressions](/docs/expression) to define features with code that can be
...
In addition to being able to make SageMaker calls, [underscore expressions](/docs/expression) now support a
...
[Underscore expressions](/docs/expression) now support datetime subtraction and the use of a new
...
[underscore expression documentation](/docs/expression#additional-functions).
...
[Underscore expressions](/docs/expression) on [windowed](/docs/aggregations) features can now include the special
...
We shipped static planning of [underscore expressions](/docs/expression). Underscore expressions
