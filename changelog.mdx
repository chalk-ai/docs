...
With automatic scaling we're able to optimize resource usage--expect to see a drop in cloud spend as a result!

## View logs and export metrics using the CLI

View logs using the Chalk CLI; inspect your log stream the same way you would from the Chalk dashboard but with the command line:

`chalk logs [--query="the same string you can put in the web ui"]`

Chalk supports continuously exporting metrics to an OpenMetrics compatible collector, you can now also view a sample of your metrics export using the CLI with `chalk metrics export`.
This helps considerably when experimenting with deploying a new collector.
...
The Data Lineage page of your Chalk dashboard lets you track your features--how each one is derived, referenced, Named Queries, etc.
In addition to the default list view, we've implemented a graph view, making it easier than ever to trace how your features are created and connected.
