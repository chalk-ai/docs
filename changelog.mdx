...
### Continuously backfill windowed aggregations

In addition to streaming data to update your windowed aggregations, you can now also use continuous
backfills that run on a schedule to update your windowed aggregations and specify when Chalk should
compute features using online resolvers or from the pre-aggregated buckets. Read more about how to use
windowed aggregations in our [docs](/docs/aggregations#continuous-backfills).
...
### Dashboard updates for viewing stack traces and recent feature values

You can now view the latest stack trace for a pod in your cluster under `Settings > Kubernetes`, which can be helpful
for debugging offline queries or scheduled runs. Cusotmers with a gRPC engine can also use the new `Values Preview` tab
on the Features page to view recently observed feature values as loaded from the offline store.
