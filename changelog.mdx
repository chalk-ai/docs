...
* We now support [DynamoDB](/docs/dynamodb) as a data source! After connecting your AWS credentials, Chalk 
  automatically supports connecting to your DynamoDB instance. Your queries will be run with
  [PartiQL](https://partiql.org/). Query away!
* The [`@online`](/api-docs#online.static) and [`@offline`](/api-docs#offline.static) resolver decorators have a new
  `static` boolean parameter. Use this parameter to flag whether the resolver is eligible for static planning.
* Underscore expressions now support the `_.chalk_window` operator for [`windowed` resolvers](/docs/aggregations). Use
  `_.chalk_window` to filter data based on the current context's time window:
```python
@features
class Transaction:
    id: int
