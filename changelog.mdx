...
With KEDA, Chalk environments can now dynamically scale up and down--even to **zero machines** when there is reduced or no workload.
With automatic scaling, we are able to optimize resource usage--expect to see a drop in cloud spending as a result!
...
The new metrics export CLI command helps considerably when experimenting with deploying a new collector.
