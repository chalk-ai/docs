...
We have shipped a GRPC engine for Chalk that drastically improves performance by **at least 2x** through improved data
...
With ChalkPy v2.38.8 or later, you can now construct a "spine SQL query" to construct a unified spine of primary
keys that enables you to make more complex offline queries. By constructing this spine, you can now query multiple
root feature sets (also called namespaces) and access the same feature multiple times from one row for each primary
key associated with that row. See an example of how to construct a spine SQL query below:
...
dataset = client.offline_query(
	spine_sql_query=query,
	output=[
		A.feature_from_featureset_A,
		B.feature_from_featureset_B
	],
	max_samples=10
)
```

### Underscore Static Planning

We have shipped underscore static planning, which enables you to define feature resolution using underscore notation
in the feature definition, that is then translated to performant C++ code. This is currently shipped for basic
arithmetic and logical operations, and we are continuing to build out more underscore functionality! See the code
snippet below for some examples of how to use underscore static planning.
...
You can also specify a required pattern for your tags by adding a `tagregex` field to your `chalk.yaml` file.
If you've added this field, your deployment tags will need to match your supplied regex. For instance:

```chalk.yaml
project: my-project-id
tagregex: latest|v[0-9]+\.[0-9]+\.[0-9]+
environments:
  default:
    runtime: python310
...
We've updated our UI for resource configuration management in the dashboard! You can now toggle your view between
...
We've expanded the functionality of our service tokens to enable role-based access control (RBAC) at both the
datasource and feature level. On the datasource level, you can now restrict a token to only access datasources
...
### New Datasources and Native Drivers

We've added integrations for Trino and Spanner as new datasources. We've also added native drivers for Postgres and
Spanner, which drastically improves performance for these datasources.
...
We've shipped statuses during incremental runs such that users can get a signal of the current high water mark of
data being updated.
...
* In addition to the changes listed above, we have also shipped a number of smaller improvements including but not
limited to:
...
![Documentation search](/img/docs-search.gif)
