...
In the Source Code viewer on the Deployments page of the dashboard, you can now search for files by name
We've extended our Dashboard's Deployment Source Code Viewer! In addition to searching
for keywords in your source code, you can now also search for your files by name.

![Source code viewer with file search](/img/view-source-file-explorer.png)

### Include query context in metadata field

The Chalk Client now offers an [include_metadata](/api-docs#ChalkClient.query.include_meta) option to associate
a key-value dictionary with a query. This can be useful for storing information such as context about the query
or for tagging and labeling queries.
