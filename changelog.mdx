...
### New feature and resolver tables in the dashboard

We have shipped new tables in the Features and Resolvers sections of the dashboard!

The new tables have compact filtering with expanded functionality–you can now filter
and sort by various resolver and feature attributes! The tables also provide column resizing
for intuitive exploration of the feature catalog.
...
with sorting built in, as well as a `Features as CSV` button to download all the feature attributes
...
You can now define features as JSON values and reference the JSON fields in feature, resolver,
and query definitions. Read more on how to define and reference JSON features
[here](https://docs.chalk.ai/api-docs#json_value).
...
In the [August 19](/docs/changelog#queries-can-reference-multiple-feature-namespaces) changelog entry, we announced
