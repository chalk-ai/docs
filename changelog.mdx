...
Outputs for offline query can now be automatically transferred to online and offline storage with the parameters
`store_online=True` and `store_offline=True` in `ChalkClient().offline_query()`.
See [the docs](https://docs.chalk.ai/api-docs#ChalkClient.offline_query) for more details
