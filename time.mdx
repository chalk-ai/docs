...
ChalkClient().offline_query(
    # Pass id 1 multiple times because we want to request it with multiple input_times
    input={User.id: [1, 1, 1]},  
    input_times=[
...
For ergonomics, you may find it easier to access query time when it is correlated with the rows of your DataFrame. To
have Chalk include query time as a column in your DataFrame, use [`output_ts`](/api-docs#ChalkClient.sample.output_ts).
