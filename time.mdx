...
By default, features are timestamped with the execution time of their resolver. You can override this behavior by
providing timestamps from your data source. This functionality can be helpful when working with an event store or
timestamped API.

The first step is to add a `FeatureTime` attribute to your feature class.
