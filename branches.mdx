...
We start out with a dataset. We may have produced this dataset manually, by calling an external API, or by executing an offline query.
...
In our Jupyter Notebook, we can execute the following in a cell to setup our notebook to point to our branch, then update our branch with the new feature and resolver.
