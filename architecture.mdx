...
import { TipGoodText } from "@/components/Tip"
...
![Velox architecture diagram](/img/architecture-velox_architecture.png)
...
Chalk is deployed with Terraform and uses common cloud primitives, making it easy to integrate deployments with your existing infrastructure.
...
In the Chalk-Hosted Deployment, both the [Metadata Plane](#metadata-plane) and [Data Plane](#data-plane)
...
In this model, the customer runs the [Data Plane](#data-plane) in its own cloud account,
and Chalk runs the [Metadata Plane](#metadata-plane) in its cloud account.
...
Chalk offers the option to self-host _both_ the [Data Plane](#data-plane) and [Metadata Plane](#metadata-plane).
In the [Customer Cloud Deployment](#customer-cloud-deployment), only the Data Plane runs in your cloud account,
whereas in the Air-Gapped Deployment, the [Metadata Plane](#metadata-plane)
joins the [Data Plane](#data-plane) in your cloud account.

There are two primary reasons that customers will choose to host the [Metadata Plane](#metadata-plane):
...
for the underlying Kubernetes resources of [Data Plane components](/docs/kube-resources-drilldown#data-plane).
