...
3. Deploy your new feature with [`chalk apply --no-promote`](/cli/apply#no-promote).
...
```bash
$ chalk query --in animal.id=Cow --out animal.sound
```
5. To query a specific version, use the `@` syntax
```bash
$ chalk query --in animal.id=Cow --out animal.sound@2
```
...
def resolver_2(id: Animal.id) -> Animal.sound:
    match id:
        ...
        case _: return "Hello"
```
2. [`chalk apply --no-promote`](/cli/apply#no-promote) to deploy.
