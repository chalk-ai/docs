...
make_stream_resolver(
    ...
    sink=Sink(
        ...
        output_features=[Item.id, Item.item_embedding],
    ),
)
...
2. The sink configuration specifies that `Item.id` and `Item.item_embedding` should be written to the `item_embeddings` stream
...
Messages are encoded as arrow feather tables.
