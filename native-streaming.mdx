...
## Skip Writing to the Store

In some cases you, you may find that your existing architecture already stores the historical log of data in a data warehouse. In cases like this, it may be helpful to not worry about writing duplicate data to the offline store from a stream and instead just use the existing data warehouse for historical look back with offline queries. This can be performed using `make_stream_resolver`'s [`skip_offline`](/api-docs#make_stream_resolver.skip_offline) parameter:
...
When working in multiple environments, there may be stream resolver you want to deploy to a specific environment, such as a new resolver in dev. This can be done using the [`environment`](api-docs#make_stream_resolver.environment) parameter:
