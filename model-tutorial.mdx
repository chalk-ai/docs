...
from chalk.features import features, Primary, DataFrame, FeatureTime, windowed, _
...
class Event:
    ...
    type: str
    ts: datetime
    ts_unix_seconds: int = F.unix_second(_.ts)
...
# Register a model namespace (first time only)
client.register_model_namespace(
    name="InteractionModel",
    description="Gradient boosting model for predicting an event's interaction probability"
)
...
result = client.query(
    ...
    output=[
        ...
        Event.user.engagement_score,
        Event.user.days_since_last_login,
        Event.user.total_logins["1d"],   # Query specific windows
        Event.user.total_logins["7d"],
        Event.user.total_logins["30d"],
    ],
)
