...
1.  `input` (required): A lambda that returns the feature that will be embedded. If the embedding model takes multiple
    ...
    If you would like to use multiple features as input, you can define a [resolver](/docs/resolver) to combine
...
1.  `max_staleness` (optional): The duration for which the embedding will be cache. By default, the embedding vector 
...
Chalk supports [INSTRUCTOR](https://github.com/xlang-ai/instructor-embedding) embedding models. When using instructor,
the `feature` lambda should return a tuple of (instruction, feature to encode) instead of just a single feature.
The instruction can be a constant string or another feature. See the available models
...
and a resolver to compute the instruction.
