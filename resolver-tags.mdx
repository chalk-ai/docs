...
SQL resolvers can also specify flags via the `flags` comment.
For example, if you wanted to use Snowflake for `Transaction` features
in a fraud context and use Redshift for the same features in a signup
context, you could write two different SQL file resolvers:

```src/resolvers/users.chalk.sql
-- type: offline
-- resolves: User
-- source: snowflake
-- tags: ["fraud"]
...
```src/resolvers/users_signup.chalk.sql
-- type: offline
-- resolves: User
-- source: redshift
-- tags: ["signup"]
