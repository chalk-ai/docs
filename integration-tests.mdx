...
You can also use the check macro to test for errors `ChalkClient`. The `check` method allows assertion on values, errors, and cache hits. Mismatches between expected
and resolved data will be printed in a table.
