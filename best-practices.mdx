...
Use the test datasource button in the chalk UI to test your datasources after they've been communicating
...
### Avoid Naming Your Datasources by Their "type", e.g. calling a Postgres datasource "postgres"
...
### Run simple online queries through the chalk CLI, but for flexibility use one of chalk's clients.
...
this will make them easier to track,

### Create and share datasets with offline queries

### poll triggered queries to make sure they've executed successfully.

## Deployment

### Code changes should be tested and queried on the branch server.

### Environment variables and data sources are picked up by new deployments or by reactivating the branch server.

### Use the same codebase for dev, staging, and prod. Distinct functionality between the three can be handled via decorators and resolvers that operate in only one feature space.
