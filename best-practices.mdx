...
Though this works, it can lead to ambiguity in SQL file resolvers if you add multiple data sources of the same type: when you have only one instance of a data source type, [Chalk lets you refer to that data source by it's type instead of its name](/fraud-3#online-data)
...
Global setup for resolver should be done through a function decorated with the [@before_all decorator](generic#initializing). This also allows for unique setups for different environments.
