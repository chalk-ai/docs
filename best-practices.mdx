...
either unpacking the nested class into basic types or defining and joining an additional Chalk feature set if the nested component is truly a separate entity. Defining a
...
If you want to apply a tag or owner to all features in a feature set, this
...
### When starting your Chalk implementation, define all your feature sets in the same file
...
Joins between feature sets can be specified in a number of different ways.
We recommend using an implicit join syntax, which
[we cover in the join section of the docs](/docs/has-one#recommended-implementation).

---

## Resolvers

### Use underscore resolvers for simple calculations

[Underscore resolvers](/docs/underscore) should be used for relatively simple calculations: your underscore resolver definitions should fit inline.
...
NamedQuery(
    ...
    owner="mary.shelley@aol.com",
    description="Return a condensed view of a book, including its title, author, year, and short description."
)
```
...
```sh
chalk query --in book.id=1 --out book.id --out book.title --out book.author --out book.year --out book.author.name --out book.short_description --staleness book.short_description=0s --tags team:analytics
```
...
Your Chalk project must contain a project configuration file named `chalk.yaml` or `chalk.yml` in its root directory.
The `chalk.yaml` file allows you to configure your project's [Docker environment](/docs/docker), Python runtime, and
[feature validations](/docs/validation). Here's a sample chalk.yaml file:
...
By default, Chalk uses [`pip`](https://pypi.org/project/pip/) to install dependencies defined in `requirements.txt`. You
can configure the requirements file name in `chalk.yaml` for each of your environments as described
[here](/docs/docker#extra-pip-dependencies).

`.chalkignore` file should include your scripts, notebooks, and tests: anything that you are not actively using in your
deployment should be put there so that non-deployment code does not clutter or interfere with your deployment.
