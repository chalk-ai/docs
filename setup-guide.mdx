...
deploys when you run [`chalk apply`](/cli/apply) (more on this later).
...
or run the [`chalk dashboard`](/cli/dashboard) command.
...
[`chalk apply --branch {BRANCH_NAME}`](/cli/apply#branch) to deploy to the [branch server](/docs/branches), which allows
