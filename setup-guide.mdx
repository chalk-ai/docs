...
1. If you don't already have a Github repository in which you will store your Chalk code, create one. Then, pick a
...
$ brew install python
```
3. Install the [Chalk command line tool](https://docs.chalk.ai/cli). The Chalk CLI allows you to create, update, and
...
The third file, `requirements.txt` should look something like this:
   ...
   You can add a file `.chalkignore` which will act just like a `.gitignore` file.
...
Having set up a basic project directory, we want to configure the data sources from which we will load data to
...
[`chalk login`](https://docs.chalk.ai/cli/login) command will open your browser and create an
...
You will automatically see all of the environments in which the email you used to login has been
provisioned as a user.
 2. Within the dashboard, navigate to `Data Sources` in the side bar and add all data sources that you will
be working with here. After you have saved a data source, you can also use the `Test Data Source` button
...
For more details on setting up data sources, [see here.](https://docs.chalk.ai/docs/integrations)

##  Define Feature Sets and Resolvers

Having set up our data sources, we now want to define the registry for this feature store by writing our
feature sets and resolvers. Each *feature set* is a Python class of features, and a *resolver* tells Chalk
how to compute the values for different features. Each feature that we write should correspond to a resolver output.

We recommend starting with a minimal feature set and building up iteratively to easily test your code along the way.
After writing some feature sets, resolvers, and tests, we would expect to see a directory structure like this:
```
root_directory/
├── src/
│  ├── resolvers/
...
You can read more in our docs about the [different kinds of features](https://docs.chalk.ai/docs/features) and the
[different kinds of resolvers](https://docs.chalk.ai/docs/resolver-overview) that you can write. If you would like
guidance on how to structure your featuresets and resolvers, please reach out in your support channel!

---

## Deploy and Query

Having defined features and resolvers, the next step is to deploy your pipelines. You can deploy to production by
using the [`chalk apply`](https://docs.chalk.ai/cli/apply) command. During development, we recommend that you use
`chalk apply --branch {BRANCH_NAME}` to deploy to the [branch server](https://docs.chalk.ai/docs/branches), which allows
...
[`chalk query`](https://docs.chalk.ai/cli/query) command or from within a notebook using the
[ChalkClient](https://docs.chalk.ai/api-docs#ChalkClient) to run `ChalkClient.query()` or `ChalkClient.offline_query()`.

This is the primary workflow for iterating on features and resolvers! Write, deploy, and query to verify whether the
feature values that you receive are the expected values. Once you have finalized your feature set and resolver
definitions, then the final step is frequently orchestration.

---

## Orchestration

Once you have verified that your feature and resolver definitions are correct, then the next step is to determine how
you want to use the corresponding feature values within your larger machine learning platform. Some users [trigger
resolvers](https://docs.chalk.ai/docs/runs) and run queries from within other orchestrated pipelines, such as Airflow.
Some users define [cron schedules](https://docs.chalk.ai/docs/resolver-cron)for their resolvers,
and [set staleness values](https://docs.chalk.ai/docs/feature-caching) on different features to
...
For a detailed tutorial on how to build a fraud model using Chalk, [see here](https://docs.chalk.ai/docs/fraud-1).
