...
correct. To refresh your token, you can run [`chalk login`](/cli/login) from the terminal using our CLI and then [`chalk config`](/cli/config)
...
## [`chalk apply`](/cli/apply) Errors

These are errors you would encounter when running [`chalk apply`](/cli/apply) or [`chalk apply --branch`](/cli/apply#branch).

### Branch server might still be starting

You would usually encounter this error when running [`chalk apply --branch`](/cli/apply#branch). In the Chalk dashboard under `Branches`, you
...
### Could not connect to server when running [`chalk apply`](/cli/apply)

If you've verified that you have a token and are properly authenticated, then you should also verify what role you
have within the Chalk environment. The `developer` role can only run [`chalk apply --branch`](/cli/apply#branch) rather than [`chalk apply`](/cli/apply).
