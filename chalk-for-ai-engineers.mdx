...
Chalk provides a full-stack solution but can also integrate seamlessly with your current infrastructure. For instance, you can use
an existing vector DB to store and retrieve your embeddings, while leveraging Chalk to evaluate new models and prompts. Chalk lets
you integrate only the pieces you need:
...
Chalk makes it easy to [cache features](docs/feature-caching). This is done by setting the `max_staleness` of a feature, which lets you specify how old a cached feature value can be before it must be recomputed. This gives you full control in balancing freshness and performance requirements.
