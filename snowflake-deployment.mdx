...
To use Snowflake as your offline store, we require within your Snowflake instance
...
you can share the password with us either through DM in plaintext, or more securely,
send us an encrypted message following [these directions here](/docs/public-key).
...
Once we have created the warehouse, database, schema, user, and role, then the next step
is to create storage integration(s). The storage integration will create an IAM entity
...
TYPE = EXTERNAL_STAGE
STORAGE_PROVIDER='s3'
STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::891377258527:role/snowflake-chalk-{organization}-dataset-bucket'
ENABLED = true
