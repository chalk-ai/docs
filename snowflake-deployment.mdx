...
- **AWS Access**: Permissions to create IAM policies and roles in your AWS account
- **Snowflake Access**: Admin or role creation privileges in your Snowflake account
- **Required Information from Chalk**:
  - AWS Account ID for the IAM role (provided by Chalk)
  - S3 bucket name to use (provided by Chalk)
  - Your Snowflake role name (e.g., `CHALK_ROLE`)
...
3. Set the trust relationship with the following policy (you will update this in Step 4):
...
5. Replace the placeholder values with the actual values from Step 3's `DESCRIBE INTEGRATION` output:
   - Replace `<STORAGE_AWS_IAM_USER_ARN>` with the actual `STORAGE_AWS_IAM_USER_ARN`
   - Replace `<STORAGE_AWS_EXTERNAL_ID>` with the actual `STORAGE_AWS_EXTERNAL_ID`

6. Save the updated trust policy
...
1. Record the Storage Integration Name (e.g., `s3-integration-chalk-{organization}-data-bucket`)
2. Share the following values with Chalk:
