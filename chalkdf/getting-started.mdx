...
2          | 2
2          | 3
>>> grouped = df.agg(["id"], df.column("value").sum().alias("value_sum_by_id"))
>>> grouped.run()
...
To add new columns or modify existing columns in your DataFrame, you can use `DataFrame.with_columns`.

```python
>>> processed_interactions = df.with_columns({"user_id": df.column("user_id"), "long_session": df.column("session_duration_sec") > 180})
...
To define more complex transformations, you can use `DataFrame.project` to take in a DataFrame as input and project
to new columns based on provided expressions. In these expressions, you can use the underscore notation `_` to
reference columns within your source DataFrame, and utilize the [`chalk.functions`](/api-docs#section-chalk-functions)
...
>>> df.filter(df.column("score") > 0.8).run()
...
To compute aggregations over groups of data, you can use `DataFrame.group_by` in combination with `DataFrame.agg`.
