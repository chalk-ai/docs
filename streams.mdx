...
Chalk supports [Kafka](https://kafka.apache.org/) and [Kinesis](https://aws.amazon.com/kinesis/) for streaming.
...
```py
from chalk.streams import KafkaSource, KinesisSource
...
```py
from chalk import features, Features
...
On this page, we will focus on mapping, which creates one Chalk feature instance per message.
...
Those where it is equal to `None` will not be processed.
...
Refer to the [aggregations](/docs/aggregations#custom-event-timestamping) page for why timezoned timestamps
...
Assuming you have the above resolver, you can pull off messages from Kinesis and just insert them into your resolver.
