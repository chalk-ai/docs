...
via has-many relationships. Like has-one relationships, you must set a join
condition to link the two feature sets.

## Recommended Implementation

Below is the simplest way to specify a join for a has-many relationship. In the example below,
a `User` is linked to potentially multiple `Transfer`s.
...
class Transfer:
    id: str
    user_id: "User.id"
    user: "User" # (optional, if you need to access the user through transaction)
    amount: float
