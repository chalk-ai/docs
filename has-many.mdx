...
In addition you can also use composite keys to join Has-Many features.
...
@features
class HospitalDepartment:
    id: str = _.name + _.hospital _name
    name: str
    hospital_name: str
    employees: DataFrame[HospitalEmployee] = has_many(lambda: HospitalDepartment.id == HospitalEmployee.hospital_department)
```
