...
Has-many relationships link a feature set to many examples of a different feature set. Like has-one
relationships, you must set a join condition to link the two feature sets.
...
class Transfer:
    id: str
    user_id: "User.id" # (note, the annotation is a string reference)
    amount: float
...
on the other side of the relation (following the above example, a user has
many transfers, but a transfer has a single user). With Chalk, you don't have
to explicitly set the join a second time. Instead, the join condition is assumed
to be symmetric and copied over. All you need to do to complete the one-to-many
relationship is to add a `User` to the `Transfer` class:
...
Many-to-many is defined by a has-many relationship on both sides of the relation.
