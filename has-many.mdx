...
single user). With Chalk, you don't have to explicitly set the join a second time. Instead,
the join condition is assumed to be symmetric and copied over. All you need to do to complete
the one-to-many relationship is to add a `User` to the `Transfer` class:
