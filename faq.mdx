...
Yes! Within the web console you can assign roles with different permissions to different users. The default roles
available are  shown below.

![rbac](./img/rbac.png  "RBAC")
...
see [here](https://docs.chalk.ai/docs/okta-scim).

### How should I set up secrets to be accessible in our deployed environments?

You can configure secrets as environment variables in the `Secrets` tab of `Settings` in your Chalk console, and
...
see [this tutorial on feature versions](https://docs.chalk.ai/docs/feature-versions)

### How does FeatureTime work? Should I override FeatureTime with timestamps from existing data sources?

Feature times act as observation timestamps for a set of feature values, which is important when working with
staleness guarantees, and incrementally loading data. Chalk provides two default methods of loading feature times.
The first is a feature annotation `FeatureTime` that serves as a `datetime.datetime` with `typing.Annotation` applied.
The second is that if you define a feature named `ts` with a type of `datetime.datetime` in a feature set, then it will
be treated as a feature time. You can override the feature time by supplying values for the `ts` feature if you
would like to use the timestamps from your existing data sources as the observation timestamps for the feature.
For more information on how feature times work, [see here](https://docs.chalk.ai/docs/time).

### How should I structure my windowed features? What kind of windows can I use?

We recommend writing [windowed features](https://docs.chalk.ai/docs/aggregations) for aggregated computations over
...
### How do I know when my resolver will run and when it will timeout?

You can set [cron schedules](https://docs.chalk.ai/docs/resolver-cron) for your resolvers, which will be parsed in
your local system's timezone. You can also set customized [timeouts for resolvers](https://docs.chalk.ai/docs/timeout).
...
[Chalk DataFrame structure.](https://docs.chalk.ai/docs/dataframe) You can do some aggregations and projections
...
offline query is a combination of checking the query run in console and the status of pods in the cluster. For more
...
See answers above under `Resolvers` and `Querying` respectfully!
...
also customize this polling using [an API to query run status.](https://docs.chalk.ai/docs/runs#query-run-status)
...
To get a glimpse of recently released features, take a look at our [changelog!](https://docs.chalk.ai/docs/changelog)
