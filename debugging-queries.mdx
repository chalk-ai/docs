...
This will execute the resolver locally, using the same input data that was used in the query. This
allows you to debug using your IDE (VS Code, PyCharm, or even pdb):
...
Use the `chalk_logger` from the `chalk.clogging` just like a standard python logger. This logger
will output to the web interface, and to configured log sinks (for example, your DataDog instance).
...
<AttributeTable>
    ...
    <Attribute field={'exception'} kind={'object?'}>
        ...
        <SubAttributeTable>
            ...
            <SubAttribute field={'stacktrace'} kind={'string'}> The stack trace produced by the
      code. </SubAttribute> </SubAttributeTable> </Attribute> <Attribute field={'feature'}
  kind={'string?'}> The fully qualified name of the failing feature, for example:
  <code>user.identity.has_voip_phone</code>. </Attribute> <Attribute field={'resolver'}
  kind={'string?'}> The fully qualified name of the failing resolver, for example:
  <code>my.project.get_fraud_score</code>. </Attribute> </AttributeTable>
...
<AttributeTable title={"Values"}>
    ...
    <Attribute field={'VALIDATION_FAILED'} kind={<a href={'#field-error'}>FIELD</a>}> A feature
    value did not match the expected schema (for example, `incompatible type "int"; expected
    "str"`) </Attribute> <Attribute field={'RESOLVER_FAILED'} kind={<a
    href={'#field-error'}>FIELD</a>}> The resolver for a feature errored. </Attribute> <Attribute
    field={'RESOLVER_TIMED_OUT'} kind={<a href={'#field-error'}>FIELD</a>}> The resolver for a
    feature timed out. </Attribute> <Attribute field={'UPSTREAM_FAILED'} kind={<a
    href={"#field-error"}>FIELD</a>}> A crash in a resolver that was to produce an input for the
    resolver crashed, and so the resolver could not run crashed, and so the resolver could not run.
    </Attribute> <Attribute field={'UNAUTHENTICATED'} kind={<a href={'#network-error'}>NETWORK</a>}>
    The request was submitted with an invalid authentication header. </Attribute> <Attribute
    field={'UNAUTHORIZED'} kind={<a href={'#network-error'}>NETWORK</a>}> The request has
    credentials that do not provide the required authorization to execute an operation. </Attribute>
    <Attribute field={'INTERNAL_SERVER_ERROR'} kind={<a href={'#network-error'}>NETWORK</a>}> An
    unspecified error occurred. </Attribute> </AttributeTable>
