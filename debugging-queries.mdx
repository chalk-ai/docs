...
They are often caused by invalid feature names in the
input or by requests that cannot be satisfied by the resolvers you
have defined.
...
When a resolver crashes, you will receive `null` value
in the response.
...
They can be caused by unauthenticated requests,
