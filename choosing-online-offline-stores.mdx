...
| Big Table   | Distributed wide-column NoSQL database   | milliseconds | horizontal |
...
costs, and Big Table for huge storage needs.
...
We generally recommend DynamoDB for high performance and Elasticache for low-latency and greater storage capacity
for large rows. However, some customers may choose to use RDS depending on their existing data platform.
...
| Store                 | Storage                     |
|-----------------------|-----------------------------|
| Google Big Query      | Columnar (BigTable-based)   |
| Amazon Redshift       | Columnar (Parquet)          |
| Snowflake             | Columnar (Micro-partitions) |
| Databricks Delta Lake | Columnar (Parquet)          |

For customers with GCP cloud deployments, we generally recommend Google Big Query for its performance in
