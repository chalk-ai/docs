...
deploy feature pipelines for machine learning. For data science workflows, Chalk can be used in
entirely in a notebook to iteratively build features and generate training data.
...
We will use the [`%%resolver`](/docs/notebook-development#resolver) magic to define SQL resolvers in our notebook.
...
In the resolvers above, we use the [`after`](/docs/window-functions#after) operator to
