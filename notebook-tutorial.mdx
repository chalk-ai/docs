---
title: Tutorial
description: Work through a fraud scoring example using Chalk in a Jupyter notebook
published: true
---
...
a dataset of credit card authorizations and build a fraud scoring model.
...
4. [Offline Queries](#offline-queries)
...
```python notebook.ipynb
from chalk.features import offline, online, Features, FeatureTime

@offline
def get_merchant_features() -> (
    Features[Merchant.id, Merchant.name, Merchant.category, Merchant.country_code]
):
...
@offline
def get_card_holder_features() -> (
    Features[CardHolder.id, CardHolder.name, CardHolder.address, CardHolder.created_at]
):
    return snowflake.query_string(
        """
        SELECT id,
...
@offline
def get_authorization_features() -> (
    Features[
        Authorization.id,
...
@offline
def get_card_features() -> Features[Card.id, Card.cardholder_id, Card.issued_at]:
    return snowflake.query_string(
        """
        SELECT id,
               cardholder_id,
...
Specifically, we will define the following features to our `Authorization` and `Card` feature sets:
...
+
+   # The average transaction amount in the last day
+   avg_transaction_amount_1d: float
+
+   # The average transaction amount in the last 7 days
+   avg_transaction_amount_7d: float
+
+   # The average transaction amount in the last 30 days
...
+
+   # The total number of transactions in the lasy day
+   count_transactions_1d: int
+
...
```python notebook.ipynb
from chalk.features import offline


@offline
def get_previous_auth_amount(
    card_id: Authorization.card_id,
    authorized_at: Authorization.created_at
) -> Authorization.previous_auth_amount_in_cents:
    query = """
        SELECT amount_in_cents as prev_auth_amount
        FROM authorizations
        WHERE card_id = :card_id
        AND created_at < :authorization_time
        ORDER BY created_at DESC
        LIMIT 1
    """
    prev_amount = snowflake.query_string(
        query=query,

        # Mapping of the args specified in the SQL query string
        args={"card_id": card_id, "authorization_time": authorized_at},

        # Mapping from column names selected to features
        fields={"prev_auth_amount": Authorization.previous_auth_amount_in_cents},
    ).one_or_none()

    return prev_amount or 0
```

Note that we use `args` to pass in the arguments to the SQL query string
and `fields` to specify the mapping from the column names selected to the
features we defined.

```python notebook.ipynb
@offline
def get_avg_transaction_amounts(
    card_id: Authorization.card_id, authorization_time: Authorization.created_at
) -> DataFrame[
    Authorization.avg_transaction_amount_1d,
    Authorization.avg_transaction_amount_7d,
    Authorization.avg_transaction_amount_30d,
]:
    query = """
        SELECT
            AVG(CASE WHEN timestamp BETWEEN :authorization_ts - INTERVAL '1 DAY' AND :authorization_ts THEN amount 0 NULL END) AS avg_1d,
            AVG(CASE WHEN timestamp BETWEEN :authorization_ts - INTERVAL '7 DAY' AND :authorization_ts THEN amount 0 NULL END) AS avg_7d,
            AVG(CASE WHEN timestamp BETWEEN :authorization_ts - INTERVAL '30 DAY' AND :authorization_ts THEN amount 0 NULL END) AS avg_30d
        FROM authorizations
        WHERE timestamp >= :authorization_ts - INTERVAL '30 DAY' AND timestamp < :authorization_ts
        AND card_id = :card_id
    """

    avg_txn_amounts = snowflake.query_string(
        query=query,
        args={"card_id": card_id, "authorization_ts": authorization_time},
        fields={
            "avg_1d": Authorization.avg_transaction_amount_1d,
            "avg_7d": Authorization.avg_transaction_amount_7d,
            "avg_30d": Authorization.avg_transaction_amount_30d,
        },
    ).one()

    return avg_txn_amounts
```
...
@offline
def get_count_1d_txns(
    txns: Card.authorizations[Authorization.id, after(days_ago=1)]
) -> Card.count_transactions_1d:
    return txns.count()
...
def get_days_since_card_created(
    ...
    delta = datetime.now() - issued_at
    return delta.days

@offline
def get_days_since_first_last_txn(
    txns: Card.authorizations,
) -> DataFrame[Card.days_since_first_transaction, Card.days_since_last_transaction,]:
    ...
    # Compute days since first and last transaction
    now = datetime.now()
    days_since_first_txn = (now - first_txn_date).days
    days_since_last_txn = (now - last_txn_date).days

    return DataFrame(
        {
            "Card.days_since_first_transaction": [days_since_first_txn],
            "Card.days_since_last_transaction": [days_since_last_txn],
        }
    )
```
