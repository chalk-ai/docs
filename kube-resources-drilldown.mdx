...
traffic will *not* flow through these components -- it will flow directly to the data plane.
...
- `engine` and `engine-grpc` deployments:
    ...
    - *If you choose to use a single replica, you will experience query downtime (502s, UNAVAILABLE responses) during pod restarts*.
- `query-job`:
...
- The specific formula is:
  - `T = (1 / (1 - U)) * (1 / mu)`
    ...
    - `U` is the utilization of the system
    - `mu` is the service rate of the system
- This means that if you have a system with a service rate of 1 request per second, and you set the target CPU to 50%, the average response time will be 2 seconds.
