...
out of the box via the "Target CPU %" setting, time-based scaling can be configured with a visualization UI, and more
advanced autoscaling can be achieved by setting up custom [KEDA scalers](https://keda.sh/docs/), controlled via the
`JSON` field in the Chalk UI.
