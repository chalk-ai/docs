...
The last component is a little more nuanced but gets to the heart of why
Chalk is valuable. Chalk lets you access your data and transform your for
two very different access patters: offline and online.

The **run condition** of a resolver specifies
the conditions under which a resolver will run. At
its coursest, this is either `online` or `offline`. We will go into more
detail about what this means later, but at a high level, online resolvers
are responsible and optimized for responding to live traffic-requests (for instance, information
about a single user or feature set), and offline resolvers are responsible for pulling
historical data for analytics or modeling.

The code of the resolver is the logic that transforms your input features into your
output features.
The properties of
...
date. To know a users name and birthday, we might not need any concrete features—
...
## Offline/Online, And Specifying Resolver Run Conditions
